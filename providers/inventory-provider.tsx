...
function buildIngredientStatusExport(
  ...
  const result: IngredientStatusExport = {};
  ...
  Array.from(ids)
    ...
    .forEach((id) => {
      ...
      result[key] = {
        ...(availableIngredientIds.has(id) ? { available: true } : {}),
        ...(shoppingIngredientIds.has(id) ? { shopping: true } : {}),
      };
    });
...
function parseIngredientStatusImport(data?: InventoryExportData): {
  ...
  const nextAvailableIds = new Set<number>();
  const nextShoppingIds = new Set<number>();
  ...
  Object.entries(statusCandidate).forEach(([rawId, rawStatus]) => {
    const id = Number(rawId);
    if (!Number.isFinite(id) || id < 0 || !rawStatus || typeof rawStatus !== 'object') {
      return;
    }

    const normalizedId = Math.trunc(id);
    const status = rawStatus as { available?: unknown; shopping?: unknown };

    if (status.available === true) {
      nextAvailableIds.add(normalizedId);
    }

    if (status.shopping === true) {
      nextShoppingIds.add(normalizedId);
    }
  });
...
export function InventoryProvider({ children }: InventoryProviderProps) {
  ...
  const importInventoryData = useCallback((
    ...
    const shouldImportIngredientAvailability = options?.importIngredientAvailability !== false;
    ...
    if (incomingIngredientStatus.shoppingIngredientIds.size > 0) {
      setShoppingIngredientIds((prev) => {
        const next = new Set(prev);
        incomingIngredientStatus.shoppingIngredientIds.forEach((id) => {
          next.add(id);
        });
        return next;
      });
    ...
    if (shouldImportIngredientAvailability && incomingIngredientStatus.availableIngredientIds.size > 0) {
      setAvailableIngredientIds((prev) => {
        const next = new Set(prev);
        incomingIngredientStatus.availableIngredientIds.forEach((id) => {
          next.add(id);
        });
        return next;
      });
