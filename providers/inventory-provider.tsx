...
export function InventoryProvider({ children }: InventoryProviderProps) {
  ...
  const [useCardView, setUseCardView] = useState<boolean>(
    () => runtimeCache.useCardView ?? false,
  );
  ...
  const applyInventoryBootstrap = useCallback(
    (bootstrap: {
      ...
      setUseCardView(bootstrap.useCardView);
  ...
  const buildBootstrapFromSnapshot = useCallback((snapshot: InventorySyncStateSnapshot) => {
    ...
    const nextUseCardView = snapshot.useCardView ?? false;
  ...
  const handleSetUseCardView = useCallback((value: boolean) => {
    setUseCardView(Boolean(value));
  }, []);
  ...
  const actionsValue = useMemo(
    () => ({
      ...
      setUseCardView: handleSetUseCardView,
