...
type SkiaImage = NonNullable<ReturnType<typeof Skia.Image.MakeImageFromEncoded>>;
...
const resizeToJpegWithWhiteBg = async (
  img: SkiaImage,
  targetWidth: number,
  targetHeight: number,
  ...
  const rect = Skia.XYWHRect(0, 0, targetWidth, targetHeight);
...
export const storePhoto = async ({
  ...
  try {
    ...
    const finalTempUri = await resizeToJpegWithWhiteBg(
      img,
