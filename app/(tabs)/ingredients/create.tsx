...
export default function IngredientFormScreen() {
  ...
  const placeholderLabel = useMemo(() => {
    if (imageSource) {
      return t('ingredientForm.changeImage');
    }

    return t('ingredientForm.addImage');
  }, [imageSource, t]);
  ...
  const ensureMediaPermission = useCallback(async () => {
    ...
    if (!canAskAgain) {
      showDialog({
        title: t('ingredientForm.mediaLibraryAccessTitle'),
        message: t('ingredientForm.mediaLibraryAccessMessage'),
  ...
  const handlePickImage = useCallback(async () => {
    ...
    try {
      ...
      showDialog({
        title: t('ingredientForm.couldNotPickImage'),
  ...
  const ensureCameraPermission = useCallback(async () => {
    ...
    if (!canAskAgain) {
      showDialog({
        title: t('ingredientForm.cameraAccessRequired'),
        message: t('ingredientForm.cameraAccessRequiredMessage'),
        actions: [{ label: t('common.ok') }],
      });
    }
  ...
  const handleTakePhoto = useCallback(async () => {
    ...
    try {
      ...
      showDialog({
        title: t('ingredientForm.couldNotTakePhoto'),
        message: t('common.tryAgainLater'),
        actions: [{ label: t('common.ok') }],
      });
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!trimmedName) {
      showDialog({
        title: t('ingredientForm.nameRequiredTitle'),
        message: t('ingredientForm.nameRequiredMessage'),
        actions: [{ label: t('common.ok') }],
      });
      return;
    }
    ...
    if (isEditMode) {
      if (numericIngredientId == null) {
        showDialog({
          title: t('ingredientForm.ingredientNotFound'),
          message: t('common.tryAgainLater'),
          actions: [{ label: t('common.ok') }],
        });
        return;
  ...
  const confirmLeave = useCallback(
    (onLeave: () => void) => {
      showDialog({
        title: t('ingredientForm.leaveWithoutSavingTitle'),
        message: t('ingredientForm.leaveWithoutSavingMessage'),
        actions: [
  ...
  const leaveScreen = useCallback(
    ...
    [isBackAction, navigation, returnToPath, returnToSource],
  );
  ...
  const handleDeletePress = useCallback(() => {
    ...
    const message = trimmedName
      ? t('ingredientForm.deleteConfirmNamed', { name: trimmedName })
      : t('ingredientForm.deleteConfirmUnnamed');

    showDialog({
      title: t('ingredientForm.deleteTitle'),
      message,
      actions: [
        { label: t('common.cancel'), variant: 'secondary' },
        {
          ...
          onPress: () => {
            ...
            if (!wasDeleted) {
              showDialog({
                title: t('ingredientForm.couldNotDelete'),
                message: t('common.tryAgainLater'),
                actions: [{ label: t('common.ok') }],
              });
              return;
  ...
  const baseDisableReason = hasBrandedVariants
    ? t('ingredientForm.baseDisableBrandedVariants')
    : isStyledIngredient
      ? t('ingredientForm.baseDisableStyledIngredient')
      : null;
  const styleDisableReason = hasStyledVariants
    ? t('ingredientForm.styleDisableStyledVariants')
    : isBrandedIngredient
      ? t('ingredientForm.styleDisableBrandedIngredient')
      : null;
    ...
    return (
      <>
          ...
          options={{
            ...
            headerLeft: () => (
              <HeaderIconButton
                ...
                accessibilityLabel={t('ingredientForm.goBack')}>
        ...
        <View style={[styles.container, styles.emptyState, { backgroundColor: Colors.surface }]}>
          <Text style={[styles.emptyMessage, { color: Colors.onSurfaceVariant }]}>{t('ingredientForm.ingredientNotFound')}</Text>
        </View>
      </>
    );
  }
    ...
    <ScrollView
      ...
      <View style={styles.photoTileWrapper}>
        <Pressable
          ...
          {imageSource ? (
            ...
            <View style={styles.placeholderContent}>
              ...
              <Text style={[styles.placeholderHint, { color: `${Colors.onSurfaceVariant}99` }]}>{t('ingredientForm.addImage')}</Text>
            </View>
          )}
          {imageSource ? (
            <View
        ...
        {imageSource ? (
          <Pressable
            ...
            accessibilityRole="button"
            accessibilityLabel={t('ingredientForm.removePhoto')}>
      ...
      <View style={sectionStyle}>
        <View style={styles.tagHeader}>
          ...
          <Pressable
            accessibilityRole="button"
            accessibilityLabel={t('ingredientForm.createTag')}
            ...
            <Text style={[styles.tagAddLabel, { color: Colors.tint }]}>{t('ingredientForm.createTag')}</Text>
          </Pressable>
        </View>
      ...
      <View style={sectionStyle}>
        ...
        <Text style={[hintStyle, { color: Colors.onSurfaceVariant }]}>{t('ingredientForm.baseHint')}</Text>
        {baseDisableReason ? (
          <Text style={[hintStyle, { color: Colors.onSurfaceVariant }]}>{baseDisableReason}</Text>
        ) : null}
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={baseIngredient ? t('ingredientForm.changeBaseIngredient') : t('ingredientForm.selectBaseIngredient')}
          ...
          {baseIngredient ? (
            ...
            <View style={styles.basePlaceholderRow}>
              <Text style={[styles.basePlaceholderText, { color: Colors.onSurfaceVariant }]}>{t('ingredientForm.none')}</Text>
            </View>
          )}
        </Pressable>
      </View>
      ...
      <View style={sectionStyle}>
        ...
        <Text style={[hintStyle, { color: Colors.onSurfaceVariant }]}>{t('ingredientForm.styleHint')}</Text>
        {styleDisableReason ? (
          <Text style={[hintStyle, { color: Colors.onSurfaceVariant }]}>{styleDisableReason}</Text>
        ) : null}
        <Pressable
          accessibilityRole="button"
          accessibilityLabel={styleIngredient ? t('ingredientForm.changeStyleIngredient') : t('ingredientForm.selectStyleIngredient')}
      ...
      <View style={[sectionStyle, styles.descriptionSection]}>
        ...
        <TextInput
          ...
          placeholder={t('ingredientForm.descriptionPlaceholder')}
          style={[
            styles.input,
