...
export function SideMenuDrawer({ visible, onClose }: SideMenuDrawerProps) {
  ...
  const createTarArchive = (files: Array<{ path: string; contents: Uint8Array }>) => {
  ...
  const handleBackupPhotos = async () => {
    ...
    try {
      ...
      if (addedCount === 0) {
        showDialogMessage('Backup failed', 'Unable to find any stored photo files.');
        return;
      }
        ...
        <Animated.View style={[drawerStyle, { transform: [{ translateX }] }]}>
          ...
          <ScrollView
            ...
            <Pressable
              ...
              <View style={[styles.actionIcon, { backgroundColor: Colors.surfaceVariant }]}>
                <MaterialCommunityIcons name="restart" size={16} color={Colors.onSurfaceVariant} />
              </View>
              <View style={styles.settingTextContainer}>
                <Text style={[styles.settingLabel, { color: Colors.onSurface }]}>Restart onboarding</Text>
                <Text style={[styles.settingCaption, { color: Colors.onSurfaceVariant }]}>
                  Replay the first-launch guided tour
                </Text>
              </View>
            </Pressable>
            <Pressable
